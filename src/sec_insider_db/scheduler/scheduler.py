...
def run_scheduler(settings: Settings, session_factory: sessionmaker, client: SecClient, engine) -> None:
    ...
    def tick() -> None:
        ...
        if _should_run_hourly(settings, now, state["last_hourly"]):
            ...
            _run_job(updater, source="hourly_update")
        if _should_run_nightly(settings, now, state["last_nightly"]):
            ...
            _run_job(updater, source="nightly_reconciliation")
...
def _run_job(updater: IncrementalUpdater, *, source: str) -> None:
    try:
        updater.run(source=source)
    except Exception:
